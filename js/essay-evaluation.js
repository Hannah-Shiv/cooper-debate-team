(function () {
  ...
  function renderRubric() {
    ...
    const completed = scoreCount();
    const progress = Math.round((completed / KEYS.length) * 100);
    dialog.querySelector(".eval-header-score").textContent = `${completed} of 7 categories · ${total()} / 35 points`;
    dialog.querySelector(".eval-head-recommendation span").textContent = state.recommendation
      ? recommendationLabel(state.recommendation)
      : "Not selected";
    ...
    dialog.querySelector(".eval-meter i").style.width = `${progress}%`;
    dialog.querySelector(".eval-meter span").textContent = `${progress}%`;
    root.querySelectorAll("[data-key]").forEach((category) => {
      const score = state.rubric[category.dataset.key];
      category.querySelector(".eval-cat-grade").textContent = score ? SCORE_LABELS[score] : "Choose level";
      category.querySelector(".eval-cat-score").textContent = score ? `${score}/5` : "— /5";
    ...
    dialog.querySelector(".eval-finalize").disabled = completed !== 7 || !state.recommendation;
  }
  ...
  async function finalize() {
    ...
    if (scoreCount() !== 7 || !state.recommendation) {
      setStatus("Score all categories and choose a recommendation before finalizing.", "error");
      return;
    }
    clearTimeout(saveTimer);
