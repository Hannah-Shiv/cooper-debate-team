...
(function () {
  ...
  function renderDetail() {
    ...
    $("detail").innerHTML = `<div class="detail-content">
      <header class="detail-heading"><div class="detail-title-row"><h2>${escapeHtml([student.firstName, student.lastName].filter(Boolean).join(" ") || "Unnamed applicant")}</h2><p class="detail-submission">${icon("clipboard", "detail-meta-icon")}Submitted: ${escapeHtml(formatDate(item.createdAt))}<b aria-hidden="true">|</b>Application ID: ${escapeHtml(item.id)}</p></div><div class="detail-status"><div class="badges">${statusBadge(decision)}</div></div></header>
