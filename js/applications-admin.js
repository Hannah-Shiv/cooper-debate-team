...
(function () {
  ...
  function applicantPillName(student) {
    const nameParts = [student.firstName, student.lastName].map(value => String(value || "").trim()).filter(Boolean);
    const fullName = nameParts.join(" ") || "Unnamed applicant";
    if (fullName.length <= 18 || nameParts.length < 2) return { displayName: fullName, fullName };
    return { displayName: `${nameParts[0]}…`, fullName };
  }
  function renderDetail() {
    ...
    const pillName = applicantPillName(student);
    ...
    $("detail").innerHTML = `<div class="detail-content">
      <header class="detail-heading"><div class="detail-title-row"><h2 title="${escapeHtml(pillName.fullName)}">${icon("person", "applicant-pill-icon")}<span class="applicant-pill-name">${escapeHtml(pillName.displayName)}</span></h2><div class="detail-inline-status badges">${statusBadge(decision)}${item.hidden === true ? hiddenBadge() : ""}</div></div>${isFinalReviewer() ? '<div class="detail-report-actions" aria-label="Global applicant reports"><button type="button" class="report-launch report-launch-stats" id="application-stats-report" aria-haspopup="dialog"><span class="report-launch-icon report-launch-stats-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M5 20V10h3v10M11 20V4h3v16M17 20v-7h3v7M3 20h19"/></svg></span><span class="report-launch-label">Stats</span><span class="report-launch-arrow" aria-hidden="true">›</span></button><button type="button" class="report-launch report-launch-scores" id="essay-scores-report" aria-haspopup="dialog"><span class="report-launch-icon" aria-hidden="true"><img src="images/application-report-essay-scores.png?v=1" alt=""></span><span class="report-launch-label">Essay Scores</span><span class="report-launch-arrow" aria-hidden="true">›</span></button><button type="button" class="report-launch report-launch-decisions" id="decision-status-report" aria-haspopup="dialog"><span class="report-launch-icon" aria-hidden="true"><img src="images/application-report-decision-status.png?v=1" alt=""></span><span class="report-launch-label">Decision Status</span><span class="report-launch-arrow" aria-hidden="true">›</span></button></div>' : ""}</header>
